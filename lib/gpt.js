...
GPT.prototype = {
  ...
  parseBackup( buffer, offset ) {
    ...
    // Parse header, adding 32 blocks to the offset, as the backup
    // has the header behind the partition table
    this.parseHeader( buffer, buffer.length - this.blockSize )

    // Parse partition tables
    this.parseTable( buffer, offset, offset + this.blockSize )
    this.parseTable( buffer, offset + this.blockSize, offset + this.tableSize )
  ...
  parseTable( buffer, offset, end ) {
    ...
    while( ( offset + this.entrySize ) <= end ) {
  ...
  relocate( newLBA ) {
    ...
    if( isPrimary ) {
      ...
      this.tableOffset = newLBA - ( this.tableSize / this.blockSize )

    }

    // Set the last usable LBA
    this.lastLBA = newLBA - ( this.tableSize / this.blockSize ) - 1
